...
const { fake, random, helpers } = require('faker');
const { find } = require('./models/Kafka');
const { nanoid } = require('nanoid');
// const aric_mock_data = require('./faker');
...
const generateFakeData = () => {
  return aric_mock_data;
};

const sendMessage = () => {
  // const userdata = faker.helpers.createCard();
  // const user_data = {
  //   ...userdata,
  //   transaction: faker.helpers.createTransaction(),
  // };
  // let faker_data = generateFakeData();
  const faker_data = {
    kafka_id: nanoid(5),
  ...
  return producer
    .send({
      ...
      messages: Array(getRandomNumber())
        .fill()
        .map(_ => createMessage(JSON.stringify(faker_data))),
    })
    .then(console.log('Sent to Kafka!', i, faker_data));
